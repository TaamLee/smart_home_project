...
const int DS_PIN = GPIO_NUM_4;
#define ON_CMD  "on"
#define OFF_CMD  "off"

float temp_value ;

static char put_res[100];
static int put_res_len = 0;
...
static void control_motor_handler(char* cmd)
{
    //check cmd
    if (!strncmp(cmd,"on",2))
    {
 ...
 */
static void
get_temperature_request(coap_resource_t *resource,
                  coap_session_t *session,
                  const coap_pdu_t *request,
                  const coap_string_t *query,
                  coap_pdu_t *response)
{
    uint8_t temp_payload;
    temp_payload = (uint8_t)(temp_value);
    printf("%d",temp_payload);
    coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
    coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 1,
                                 (const uint8_t *)&temp_payload,
                                 NULL, NULL);
}

static void
control_motor_request(coap_resource_t *resource,
                  coap_session_t *session,
                  const coap_pdu_t *request,
                  const coap_string_t *query,
                  coap_pdu_t *response)
{
    ...
    if (size == 0) {      /* re-init */
        ESP_LOGI(TAG,"error put request\n");
    } else {
        put_res_len = size;
        memcpy (put_res, data, put_res_len);
        coap_pdu_set_code(response, COAP_RESPONSE_CODE_CONTENT);
        coap_add_data_large_response(resource, session, request, response,
                                 query, COAP_MEDIATYPE_TEXT_PLAIN, 60, 0,
                                 (size_t)put_res_len,
                                 (const u_char *)put_res,
                                 NULL, NULL);
        ESP_LOGI(TAG,"motor status: %s\n",put_res);
        control_motor_handler(put_res);

    }
}

static void
delete_request(coap_resource_t *resource,
                     coap_session_t *session,
                     const coap_pdu_t *request,
                     const coap_string_t *query,
                     coap_pdu_t *response)
...
static void coap_example_server(void *p)
{
    ...
    while (1) {
        ...
        resource = coap_resource_init(coap_make_str_const("control_motor"), 0);
        if (!resource) {
            ESP_LOGE(TAG, "coap_resource_init() failed");
            goto clean_up;
        }
        coap_register_handler(resource, COAP_REQUEST_GET, get_temperature_request);
        coap_register_handler(resource, COAP_REQUEST_PUT, control_motor_request);
        coap_register_handler(resource, COAP_REQUEST_DELETE, delete_request);
...
void app_main(void)
{
    ...
    xTaskCreate(read_data_ds18b20_task, "coap", 8 * 1024, NULL, 5, NULL);
